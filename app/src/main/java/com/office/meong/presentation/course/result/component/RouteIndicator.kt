...
fun RouteIndicator(
    ...
    routeIndicatorType: RouteIndicatorType = RouteIndicatorType.START
) {
    val routeText = when (routeIndicatorType) {
        RouteIndicatorType.START ->
            "${RouteIndicatorType.START.label}까지 약 $routeLength km"

        RouteIndicatorType.BETWEEN ->
            "${RouteIndicatorType.BETWEEN.label} 약 $routeLength km"

        RouteIndicatorType.END ->
            "${RouteIndicatorType.END.label}까지 약 $routeLength km"
    }
    ...
    Row(
        ...
        Row(
           modifier = Modifier
               .noRippleClickable(onClick = onRouteClick)
...
@Preview
@Composable
private fun RouteIndicatorPreview() {
    MeongTheme {
        RouteIndicator(
            routeLength = "1.2",
            onRouteClick = {},
